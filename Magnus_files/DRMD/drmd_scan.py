...
# DRMD scan settings
# =============================================================================
# Keep these fixed during the delta_Neff scan
F_IDM_FIXED = 0.03
Z_STOP_FIXED = 5.0e4
G_AH_FIXED = 1.0e9

# Scan values
DELTA_NEFF_VALUES = [0.0, 0.2]#, 0.4, 0.8]
...
# =============================================================================
print("\n==============================")
print("DRMD delta_Neff_drmd scan")
print("==============================")
print(f"Fixed f_idm_drmd      = {F_IDM_FIXED}")
print(f"Fixed z_stop          = {Z_STOP_FIXED}")
print(f"Fixed G/(aH)_ini      = {G_AH_FIXED}")
print(f"Scanned delta_Neff    = {DELTA_NEFF_VALUES}")

for dNeff, p in param_sets:
    print(f"\n--- Parameters: delta_Neff_drmd = {dNeff} ---")
    for k, v in p.items():
        print(f"{k}: {v}")
...
models = []
for dNeff, p in param_sets:
    label = f"delta_Neff_drmd = {dNeff}"
    c = run_mine(p, label)
    models.append((dNeff, c))
...
pk_data = {}
for dNeff, c in models:
    pk_data[dNeff] = pk_safe(c, K_1MPC, Z_PK) * h_ref**3

Pk_ref = pk_data[DELTA_NEFF_VALUES[0]]
...
# Top panel
for i, dNeff in enumerate(DELTA_NEFF_VALUES):
    ax1.loglog(
        k_hMpc,
        pk_data[dNeff],
...
ax1.set_title("DRMD scan: varying delta_Neff_drmd")
ax1.grid(True, which="both", ls=":")
ax1.legend()

# Bottom panel
for i, dNeff in enumerate(DELTA_NEFF_VALUES):
    ratio = pk_data[dNeff] / Pk_ref
    diff_pct = 100.0 * (ratio - 1.0)
    ax2.semilogx(
        k_hMpc,
        diff_pct,
        ls=linestyles[i % len(linestyles)],
        label=f"{dNeff} / {DELTA_NEFF_VALUES[0]}"
    )
...
ax2.legend(title=r"$\Delta N_{\rm eff}^{\rm drmd}$ ratio")
...
# Summary
# =============================================================================
print("\nSummary relative to baseline delta_Neff_drmd = 0.0:")
for dNeff in DELTA_NEFF_VALUES[1:]:
    ratio = pk_data[dNeff] / Pk_ref
    diff_pct = 100.0 * (ratio - 1.0)
    print(
        f"  delta_Neff_drmd = {dNeff}: "
...
print("\nInterpretation:")
print("  - This scan isolates the effect of extra dark radiation.")
print("  - Larger delta_Neff_drmd should generally increase early radiation density.")
print("  - Compare both the overall suppression and any oscillatory features.")
